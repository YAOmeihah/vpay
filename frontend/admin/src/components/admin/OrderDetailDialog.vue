...
<template>
  ...
  >
    <el-descriptions v-if="order" :column="1" border>
      ...
      <el-descriptions-item label="创建时间">{{ formatUnixTimestamp(order.create_date) }}</el-descriptions-item>
      <el-descriptions-item label="支付时间">{{ formatUnixTimestamp(order.pay_date) }}</el-descriptions-item>
      <el-descriptions-item label="关闭时间">{{ formatUnixTimestamp(order.close_date) }}</el-descriptions-item>
    </el-descriptions>
    <template #footer>
